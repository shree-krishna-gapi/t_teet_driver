...
//  Circle circle;
  ...
  static final CameraPosition initialLocation = CameraPosition(
    ...
    target: LatLng(27.7095913,85.3141664),
    zoom: 14.0,
  );
...
// two points
  ...
  //     'AIzaSyBdeTQAqqsYBvAei69ZI1eV70Bk-M7hUgQ', // Google Maps API Key
  //     PointLatLng(27.6765179, 85.3471198),
  //     PointLatLng(27.7097955, 85.3264372),
  ...
  void initState() {
    ...
    //       print('************');
    //       print(data[key]['gender']);
    //       print('hello world');
    //       setState(() {
    //         _markers.add(
    //             Marker(
    //                 markerId: MarkerId(key),
    //                 position: LatLng(27.6730713,85.4227966),
    //                 icon: maleIcon, onTap:()=>markerFunction(key)
    //             )
    //         );
    //       });
    //     } else {
    ...
    //               icon: femaleIcon, onTap:()=>markerFunction(key)
    //           )
    //       );
    //     }
    //
  ...
  markerFunction(String phoneNumber) {
    print('you tab $phoneNumber');
  }
  @override
...
//      print("location ${currentLocation.latitude} ${currentLocation.longitude}");
  ...
  getMarker() async {
    firestore
        ...
        .snapshots()
        .listen((data) => data.documents.forEach((onlineUser) {
      setState(() {
        _markers.add(
            Marker(
                markerId: MarkerId(onlineUser['id']),
                position: LatLng(onlineUser['fromlat'],onlineUser['fromlng']),
                icon: maleIcon,
                onTap:() {
                  getRoute(onlineUser['userid'],onlineUser['fromlat'],onlineUser['fromlng'],
                  onlineUser['tolat'],onlineUser['tolng']);
                }
            )
        );
      });
  ...
  getRoute(String phone, double fromLat, double fromLng, double toLat, double toLng)async {
    print('a a a a');

    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
      'AIzaSyBdeTQAqqsYBvAei69ZI1eV70Bk-M7hUgQ', // Google Maps API Key
      PointLatLng(fromLat, fromLng),
      PointLatLng(toLat, toLng),
      // travelMode: TravelMode.transit,
    );
    if (result.points.isNotEmpty) {
      result.points.forEach((PointLatLng point) {
    ...
    // PolylineId id = PolylineId(phone);
    // Polyline polyline = Polyline(
    //   polylineId: id,
    //   color: Colors.red,
    //   points: polylineCoordinates,
