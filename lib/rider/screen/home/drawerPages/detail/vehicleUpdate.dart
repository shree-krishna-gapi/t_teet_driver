...
class VehicleUpdate extends StatefulWidget {
  VehicleUpdate({this.index,this.image,this.name,this.model,this.description});
  final String image;
  final String name;
  final String model;
...
class _VehicleUpdateState extends State<VehicleUpdate> {
  ...
  setText(){
    ...
    setState(() {
      ...
      vehicleName.text = widget.name;
      vehicleModel.text = widget.model;
      vehicleDescription.text = widget.description;
    });
    imageDownloadPath = widget.image;
  }
                      ...
                      child: Stack(
                        children: <Widget>[
                          vehiclePhoto != null ? ClipRRect(borderRadius: BorderRadius.circular(11),
                              child:Center(child: Image.file(vehiclePhoto,fit: BoxFit.cover,height: 90,width: double.infinity,))):CachedNetworkImage(
              ...
              child: Container(
                ...
                child: TextFormField(
                  ...
                  decoration: InputDecoration(
                    ...
                    filled: true,
                    suffixIcon: IconButton(
                      onPressed: () => vehicleDescription.clear(),
                      icon: Icon(
                        Icons.clear,
                        color: Colors.black,
                      ),
                    ...
                    onTap: ()async {
                      ...
                      if(imageDownloadPath == '' || imageDownloadPath == null) {
                        ...
                        if(connection) {
                          ...
                          CollectionReference collectionReference = Firestore.instance.collection('vehicles').document(phone).collection('test');
                          QuerySnapshot querySnapshot = await collectionReference.getDocuments();
                          querySnapshot.documents[widget.index].reference.updateData(demoData);
                          Navigator.of(context).pop();
                        }
                        else {
                          ToastManager().fromValidationError(
